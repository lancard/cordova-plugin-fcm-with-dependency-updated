...
This method is specifically implemented on IOS due to specific payload format ([src/FCMPlugin.d.ts](https://github.com/andrehtissot/cordova-plugin-fcm-with-dependency-updated/blob/master/src/FCMPlugin.d.ts)).
...
```bash
cordova plugin add cordova-plugin-fcm-with-dependency-updated --variable ANDROID_DEFAULT_NOTIFICATION_ICON="@mipmap/notification_icon"
```
