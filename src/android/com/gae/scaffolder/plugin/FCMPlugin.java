        ...
        try {
            if (action.equals("ready")) {
                ...
                cordova.getActivity().runOnUiThread(new Runnable() {
                    public void run() {
                        try {
                            cordova.getActivity().requestPermissions(new String[] { Manifest.permission.POST_NOTIFICATIONS }, 1020);
                        } catch(Exception e) {
                        }
                    }
                });
            ...
            public void run() {
                try {
                    if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.S) {
                        callbackContext.success(
                            ContextCompat.checkSelfPermission(cordova.getActivity().getApplicationContext(), Manifest.permission.POST_NOTIFICATIONS)
